...
\begin{tabular}{lp{0.65\textwidth}}
  ...
  \\
  {\tt --table-based} & generate a table-based lexer.\\
  \\
  {\tt --fn-based} & generate a lexer that represents states as functions and transitions as tail calls.\\
  \\
...
\GNextC
	{\T{.}}			{wildcard (all single charcters except \texttt{{$\backslash$}n})}
\GNextC
...
\GNextC
	{re re}			{concatenation}
\GNextC
	{\T{\^{ }} re}		{negation} %(any string not matched by \textit{re})}
\GNextC
...
Note that \texttt{lex\_result} must be defined as a type using the \kw{defs} directive.
